...
### References
Links that source the intent — tickets, musings, product decisions, discussions. One line each with a short label.
...
## Assumptions
Beliefs the plan depends on that could turn out wrong — the first things to sanity-check if the work stalls. Distinct from Notes (observed facts): Assumptions are bets.

## Open Questions
Known unknowns to resolve before implementation can start. Note each answer alongside its question.
