...
3 reviewers per pass, **one per lens**, launched in a single message with multiple Agent tool calls (do NOT use `run_in_background` — foreground in one batch so all results return together). The main session does not review or gather context during this phase — proceed to Phase 2 once reviewers return.
...
Use different models across the 3 slots (e.g. haiku + sonnet + opus) for cross-model bias variation on top of lens diversity. Drop a focused lens if clearly not applicable and use that slot for a second General reviewer.
